      ...
      <ul className="riel -mx-4 mt-3 flex gap-3 overflow-x-auto px-4 pb-2">
        {profesionales.map((p) => (
          <li key={p.id} className="w-56 shrink-0">
            {/* A su pantalla, no a la lista. Antes esto era
                `/profesionales#p-<id>`: el directorio entero para enseñar a
                una persona, y un ancla que acierta si la lista ya está
                pintada y aterriza en cualquier parte si todavía está
                cargando. Ahora hay un destino, y encima de la portada se
                abre como hoja. */}
            <Link
