...
Contexto permanente del proyecto, para cualquier IA que trabaje aquí —Claude
Code, Antigravity/Gemini, o quien siga—. Léelo completo antes de cualquier
tarea. `CLAUDE.md` y `GEMINI.md` redirigen aquí y solo añaden lo específico
de cada CLI.
...
Este repo se trabaja con más de una IA a la vez: hoy Claude Code como
arquitecto y Antigravity/Gemini como constructor. El protocolo completo —
economía de tokens, matriz de responsabilidades, formato de commit por rol y
los dos prompts base— vive en `docs/multiagente/protocolo.md`. Los prompts
base están en `docs/multiagente/prompts/`.

Regla corta: Claude Code diseña el blueprint y audita el diff, no escribe la
implementación completa ni corre `npm run build` / `npm test`. Antigravity o
Gemini implementan sobre ese blueprint, corren lint/build/tests hasta que
pasen en verde y actualizan el plan de tareas. Ninguna IA se salta las reglas
de este archivo ni el criterio de ADR de `docs/decisiones/LEEME.md` por
cuenta propia.
